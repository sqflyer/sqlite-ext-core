...
call :build_c
if %ERRORLEVEL% neq 0 exit /b %ERRORLEVEL%
echo Running Go concurrency and isolation stress test with C hybrid state extension...
set EXT_PATH=%~dp0bin\c_ext\libmyext.dll
cd go_loader
..\bin\concurrency_test.exe
if %ERRORLEVEL% neq 0 ( cd .. & exit /b %ERRORLEVEL% )
echo Running Go dynamic/lazy loading test with C hybrid state extension...
..\bin\lazy_test.exe
if %ERRORLEVEL% neq 0 ( cd .. & exit /b %ERRORLEVEL% )
cd ..
goto :eof
