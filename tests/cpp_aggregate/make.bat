...
cl /nologo /O2 /W4 /I"../../include" /I"%SQLITE_INC%" /std:c++14 /GR- /EHs-c- /Fe:bin\test_aggregate.exe test_aggregate.cpp "%SQLITE_LIB%" /link /NODEFAULTLIB:msvcprt.lib /NODEFAULTLIB:libcpmt.lib
if %ERRORLEVEL% neq 0 exit /b %ERRORLEVEL%

cl /nologo /O2 /W4 /I"../../include" /I"%SQLITE_INC%" /std:c++14 /GR- /EHs-c- /Fe:bin\test_aggregate_state.exe test_aggregate_state.cpp "%SQLITE_LIB%" /link /NODEFAULTLIB:msvcprt.lib /NODEFAULTLIB:libcpmt.lib
if %ERRORLEVEL% neq 0 exit /b %ERRORLEVEL%

bin\test_aggregate.exe
if %ERRORLEVEL% neq 0 exit /b %ERRORLEVEL%

bin\test_aggregate_state.exe
if %ERRORLEVEL% neq 0 exit /b %ERRORLEVEL%
