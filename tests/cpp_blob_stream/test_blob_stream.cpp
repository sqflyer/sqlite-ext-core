...
void test_blob_buffer_integration() {
    printf("3. Testing SqliteBlobStream + duo::Bytes Integration...\n");
    ...
    // 1. Write from a duo::Bytes directly into the BLOB
    {
        SqliteBlobStream stream(db, "main", "files", "data", 1, true);
        assert(stream);
        ...
        // Also test writing via duo::BytesView
        duo::BytesView bv(write_buf.data(), write_buf.size());
        assert(stream.write(bv, 0) == SQLITE_OK);
    }
    
    // 2. Read from the BLOB directly into an auto-expanding duo::Bytes
    {
        SqliteBlobStream stream(db, "main", "files", "data", 1, false);
        assert(stream);
        ...
        assert(read_buf.size() == pre_fail_size);
    }
}

void test_blob_buffer_advanced() {
    printf("4. Testing Advanced SqliteBlobStream + duo::Bytes Scenarios...\n");
