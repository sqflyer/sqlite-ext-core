...
- **Fallible Allocation Functions**: `sqlite_try_new<T>(args...)` and `sqlite_try_new_array<T>(count)` safely return `SqliteResult<T*>` with `SQLITE_NOMEM` on allocation failure, completely eliminating unhandled null dereferences.
...
- **24-Byte Small Buffer Optimization (SBO) & In-Situ UUID**: Stores primitives, inline strings ($\le 21$B), inline blobs ($\le 22$B), and raw 16-byte binary UUIDs (`InlineUuidRep`) in-situ with zero heap allocations.
- **Orthogonal UUID Formats & Zero-Copy Comparisons**: `SqliteUuidUtil::UuidFormatFlags` (`UUID_FORMAT_BLOB`, `UUID_FORMAT_TEXT`, `UUID_FORMAT_HYPHENS`, `UUID_FORMAT_UPPERCASE`, `UUID_FORMAT_BRACED`, `UUID_FORMAT_STANDARD`) with format-agnostic zero-copy equality (`uuid_equal`) and canonical 64-bit xxHash3 hashing across all $6 \times 6$ format permutations.
- **Unified Value Containers**: Specialized stack tuples (`SqliteValueTuple<N>`) and adaptive SBO vectors (`SqliteValueVec<N>`) provide RAII-managed contiguous `SqliteValueOwned` buffers with 100% stack data density and heap inspection (`is_heap()`, `is_inline()`).
...
- **Accurate Collation & Type-Safe Accessors**: Fully conforms to official SQLite collation sorting rules (`NULL < NUMERIC < TEXT < BLOB`), complete with stable `NaN` sorting constraints and type-safe `as_int64()`, `as_double()`, `as_bool()` conversions.
...
#### Key Features:
- **`SqliteValueTuple<N = 0>` ($N \in [1..8]$ Stack, $N = 0$ Direct Heap)**: Exact $N \times 24\text{B}$ in-situ stack footprint (24B, 48B, 72B, 96B, 192B) for $N \in [1..8]$ with **0 heap allocations** and 0 capacity overhead. $N = 0$ (default `SqliteValueTuple<>`) provides an immutable-width direct heap tuple where size is passed as constructor argument. Features `front()`, `back()`, `at()`, `fill()`, `swap()`, and bidirectional forward/reverse iterators.
- **`SqliteValueVec<N = 0>` ($N \in [1..8]$ SBO, $N = 0$ Direct Heap)**: Standard `std::vector` compliant container. Operates as a 24-byte aligned in-situ Small Buffer Optimized (SBO) stack vector for $N \in [1..8]$, spilling dynamically to heap when resized $> N$, or as an unbounded direct heap vector for $N = 0$ (default `SqliteValueVec<>`). Implements complete `std::vector` modifiers: `insert()`, `erase()`, `assign()`, `resize(count, val)`, `shrink_to_fit()`, `capacity()`, `is_heap()`, `is_inline()`, and `swap()`.
...
- **Single-Burst SIMD Initialization**: Container constructors leverage a pre-populated static 192-byte array of 8 canonical `SQLITE_NULL` instances (`static_null_array()`), lowered by Clang/GCC/MSVC directly into vector register operations (`vmovups`) executing in 1–2 CPU clock cycles (~0.3–0.6 ns).
...
- **Generic $9 \times 9 = 81$ Compile-Time Dispatch & Triangular Bound Pruning**: `SQLITE_DISPATCH_1D_8`, `SQLITE_DISPATCH_2D_8X8` ($9 \times 9 = 81$ combinations across $N \in 0..8$), `SQLITE_DISPATCH_ROW_KEY_COLS_8X8` (turnkey dispatcher auto-routing 0-PK tables via `SQLITE_DISPATCH_1D_8` with `KeyN = 0` and composite PK tables via lower-triangular bound pruning $1 \le KeyN \le ColsN$ yielding 45 valid combinations vs 81), `SQLITE_DISPATCH_VALID_2D`, `SQLITE_WITH_ROW_OWNED_1D`, and `SQLITE_WITH_KEY_VAL_OWNED_8X8` expand runtime column counts to compile-time `constexpr` specializations and stack-backed `SqliteRowOwnedWrapper` spans.
...
#### Documentation:
- [Small Buffer Optimizations Architecture](SBO_OPTIMIZATIONS.md)
- [Value Containers README](docs/VALUE_CONTAINERS_README.md)
- [Value Containers Architecture](docs/VALUE_CONTAINERS_ARCHITECTURE.md)
...
- **Composite xxHash3 Synthesis**: `SQLITE_DERIVE_ARRAY_HASH` synthesizes 64-bit composite hash computation with $O(1)$ scalar fast-paths and multi-column combiner loops.
...
- **Polymorphic Column Multiplexing**: Automatically routes scalar values, `SqliteValueOwned`, `SqliteValueTuple<N>`, and `SqliteValueVec<N>` to the SQLite output context.
...
### 12. DuoSTL Linear Containers (`stl/duo_linear.hpp`)
Provides `duo::String`, `duo::Bytes`, `duo::StringView`, and `duo::BytesView` as high-performance, `-nostdlib++` compliant Small Buffer Optimized (24-byte SBO) linear containers.

#### Key Features:
- **Zero-Exception OOM Safety**: All mutating methods (`reserve()`, `append()`, `push_back()`, `resize()`) return `bool` (`false` on OOM, `true` on success) without exception overhead.
- **24-Byte SBO Efficiency**: Inlines byte buffers and strings up to 23 bytes without heap allocation.
- **xxHash3 Hashing**: Natively integrates with DuoSTL's 64-bit xxHash3 hashing engine.
- **Zero-Allocation Views**: `duo::StringView` and `duo::BytesView` (`SpanView<uint8_t>`) provide lightweight 16-byte non-owning slices with zero-copy binding and comparison.
- **Direct SQLite Value Binding**: Overloads in `SqliteValueOwned`, `SqliteValueView`, `SqliteStatement`, and aggregate contexts bind and retrieve `duo::String`, `duo::Bytes`, and `duo::BytesView` directly.
...
make test-cpp-value
make test-cpp-value-containers
make test-cpp-row
make test-cpp-udf
...
make.bat test-cpp-value-containers
