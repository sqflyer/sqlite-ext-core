...
| **`SqliteValueOwned`** | **24 Bytes** | Owning (RAII) | In-Situ Multi-Layout Union / Heap (`sqlite3_value_dup`) | Strings $\le 21$B<br>Blobs $\le 22$B<br>Raw UUIDs 16B | Offset 22 (Zero-branch) | Native byte (Offset 12) | `SqliteValueHash`<br>`SqliteValueEqual`<br>`SqliteValueLess` | General-purpose SQLite value storage, map keys, accumulator state, and cache entries. |
...
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :--- |
| **`SqliteValueTuple<N>`** ($N \in [1..8]$) | **$N \times 24$ Bytes** | Owning (RAII) | Exact in-situ Stack Array | Full Stack ($N$ columns) | $64 / (N \times 24)$ tuples / line | `for (const auto& v : tuple)` | `SqliteRowHash`<br>`SqliteRowEqual`<br>`SqliteRowLess` | Fixed-arity Primary Keys, Composite Index Keys, and Fixed-Schema Records ($0\text{ mallocs}$). |
...
| **`SqliteValueVec<N>`** ($N \in [1..8]$) | **$N \times 24$ Bytes** | Owning (RAII) | Adaptive In-Situ Stack SBO $\le N$, Spills to Heap if $> N$ | Up to $N$ columns in-situ | $64 / (N \times 24)$ vectors / line | `for (const auto& v : vec)` | `SqliteRowHash`<br>`SqliteRowEqual`<br>`SqliteRowLess` | Dynamic payload rows, non-PK value columns, variable-length scratch vectors with reversible stack/heap lifecycle. |
